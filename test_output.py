...
Volvo,XC90,3,suv,hybrid,automatic,32000,42000
BMW,X5,4,suv,diesel,automatic,40000,45000
Subaru,Outback,2,wagon,petrol,automatic,22000,33000
Honda,Pilot,1,suv,petrol,automatic,18000,38000
Toyota,Highlander,2,suv,hybrid,automatic,27000,39000'''
...
def generate_response(prompt: str) -> str:
    ...
    try:
        ...
        for category in categories:
            ...
            result = classifier(
                sequences=prompt,
                candidate_labels=[category, f"not {category}"],
                multi_label=False
            )
            print(f"Result: {result}")
            if result["labels"][0] == category and result["scores"][0] > 0.7:
...
def filter_cars(df: pd.DataFrame, requirements: List[str]) -> pd.DataFrame:
    ...
    print(f"\nFiltering with requirements: {requirements}")
...
result = get_car_recommendation(test_query, df)
print("\nFinal result:")
print(result) 